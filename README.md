...
The WebAssembly [Component Model](https://github.com/WebAssembly/component-model) has chosen to only support well-formed strings, even though it could have chosen to support `DOMString` as well. Unlike ECMAScript and similar Java-like languages, some other programming languages, many data encodings, network interfaces, filesystem interfaces, etc. support only well-formed strings. Interfacing JavaScript strings with such APIs is a common use case that therefore suffers from conversion burdens. In particular because conversion from `DOMString` to `USVString` is lossy (common options are to replace unpaired surrogates or to throw an error) there is a regular need for string validation both within the platform and for certain userland use case scenarios.
